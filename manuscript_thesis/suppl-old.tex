...
{\LARGE\noindent Supplemental material for article: Predicting off-target binding
...
Figure \ref{fig:calplots_all}: Calibration plots for all targets.

Figure \ref{fig:valplots_all_0.8}: Predicted versus observed labels, at confidence level 0.8, for all targets.

Figure \ref{fig:valplots_all_0.9}: Predicted versus observed labels, at confidence level 0.9 ,for all targets.

Figure \ref{fig:workflow_detailed_fillup_vs_not}: Detailed workflow graph for
the workflow comparing the effect of extending target datasets with assumed
non-actives.

Figure \ref{fig:workflow_detailed_wo_drugbank}: Detailed workflow graph for the
workflow where DrugBank compounds were removed.
