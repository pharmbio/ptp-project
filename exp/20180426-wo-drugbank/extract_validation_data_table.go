...
func main() {
	...
	for confIdx, confLevel := range []string{"0p9", "0p8"} { // We use 'p' instead of '.' to avoid confusion in the file name
		...
		// ------------------------------------------------------------------------
		extractCmdTpl := `echo -e "a_pred_both\ta_pred_a\ta_pred_n\ta_pred_none\tn_pred_both\tn_pred_a\tn_pred_n\tn_pred_none\tsens_excl\tsens_incl\tspec_excl\tspec_incl" > {o:valstats} \
			&& cat %s \
