...
workflow is shown in figure \ref{fig:workflow_graph_wodrugbank}.  The workflow
used for comparison of metrics when filling up with assumed non-actives versus
not filling up, is similar in principle, with a few differences.

\begin{figure}[h!]
\includegraphics[width=\textwidth]{figures/workflow_graph_wodrugbank.pdf}
    \caption{Directed graph picture of the modeling workflow used to generate
    the models for the workflow where a set of DrugBank compounds were withheld
    from ExcapeDB. The workflows for the comparison of filled up datasets
    versus non-filled up are similar.}
    \label{fig:workflow_graph_wodrugbank}
\end{figure}
