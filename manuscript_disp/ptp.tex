...
allowing to compare and search for compounds with similar profile. Most
...
Chemicals were represented by the signature molecular descriptor and we
used support vector machines as the underlying machine learning method.
With conformal prediction, results from predictions are in the form of
prediction intervals.  The full pre-processing and model training process
is openly available as a scientific workflow on github, rendering it fully
reproducible.  We illustrate the usefulness of the methodology on a set of
compounds extracted from DrugBank.  The resulting models are published
online and are available via a graphical web interface and an OpenAPI
...
activity value for the same target (Gene ID) and SMILES, were completely removed.
...
\caption{The panel of targets used in this study, identified by Gene ID.
...
Efficiency metrics (OF and CAOF) for all models built on Dataset1 (without
adding assumed non-actives) are shown in figure~\ref{fig:allmodels_nofillup}.
In figure \ref{fig:allmodels_fillup}, the same metrics are shown for models
built on Dataset2 (with added assumed non-actives).
...
improve the efficiency of models trained on these datasets. This strategy was
chosen for the remaining analysis workflows.
...
\includegraphics[width=\textwidth]{figures/final_models_20171106_nofillup.png}
    \caption{All models without fill up with assumed non-actives. The targets
...
drug statuses in DrugBank statuses such as ``investigational''.
...
for confidence levels 0.8 and 0.9 respectively. We observe....
...
\end{figure}
    ...
    \inlinetodo{I would like to see a figure of 3-5 profiles (only the bottom
    right in the web page extracted) for some compounds with their chemical
    structure, where we see the intended targets show up. This figures can be
    manually drawn if needed with the target profile copied into the figure. I
    am sure that Jonathan can do something nice. Also, some text about it in
    the main manuscript.}
...
All models based on Dataset2 were published as microservices on an OpenShift
cluster and a web page federating all the models were created. At the top of
the web page (See figure~\ref{fig:web}) is an instance of the JSME
editor~\cite{Bienfait2013} in which the user can draw a molecule. As the user
draws the molecule the web page extracts the SMILES from the editor and sends
it to the model services to make predictions based on all available models. The
user can set a threshold for the confidence and get visual feedback on whether
the models predict the drawn molecule as active or non-active with the chosen
confidence.  To the right is also a graphical profile in the form of a bar plot
where confidence as binding is drawn in the positive direction and confidence
for non-binding is the other direction. Hovering it will give information about
