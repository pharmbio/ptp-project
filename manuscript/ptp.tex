...
\def\firstAuthorLast{Lamp's {et~al.}} %use et al only if is more than 1 author
...
allowing for comparing and searching for compounds with similar profiles. Most
...
process is openly available as scientific workflows on GitHub, rendering it
...
pre-processing.  The first step comprised extracting data on binding association
...
\texttt{--proper-train} parameter, as documented in the CPSign
...
of 10 models were then trained for each target using the CPSign software using
the \texttt{train} command.  The cost value used was the one obtained from the
hyper-parameter tuning.  The observations added as `assumed non-actives' were
not included in the calibration set to avoid biasing the evaluation.
...
figure~\ref{fig:workflow_graph_clean}, which shows the workflow used for
comparing efficiency metrics when adding assumed non-actives, versus when not
adding them. More detailed workflow graphs are available in the supplemental
...
Archive (JAR) files, which were also build into similarly portable Docker
containers. The model JAR files contain inside an audit log produced by
SciPipe, containing a trace of the workflow (all the shell commands and
parameters) used to produce them. They are available for download at~\cite{ModelsZenodo}
...
\begin{figure}[h!]
    ...
    \begin{minipage}[t]{0.38\textwidth}
        ...
        These were not extended with assumed non-actives.
...
This behavior might seem backwards, but at a higher confidence the predictor
...
specification is a standardization for how REST APIs are described, it means
...
page,
%(such as our reference page on \$URL\todo{add url to page here}
...
previous research on the use of predicted target profiles
earlier~\cite{Awale:2017is,Yao:2016ij}, further research is needed to maximize
their usefulness and also integrate with other types of in vitro and in silico
...
profile or targets to illustrate multi-target binding for chemical compounds.
