...
\title[Probabalistic Target Profiles]{Probabalistic Target Profiles}
...
probability for the query \todoil{candidate?} compound to interact with each target. The
probabilities are calculated using the Cross Venn-Aber\todoil{Cross-Conformal?} methodology [REF2],
with support vector machines for modeling and chemical structures described
by the signature descriptor [REF3]. We study profiles for different sets of
targets, including a subset of the minimal panel of 44 targets for broad early
hazard assessment suggested by Bowes et al.[REF1], but also the
applicability of larger as well as focused target sets. The resulting
method is available as an online Web service via an API, and we also make
the complete workflow for reproducing the study together with all models
publicly available.


\tiny
 \keyFont{ \section{Keywords:} machine learning, predictive modelling, liblinear, qsar, molecular signatures} %All article types: you may provide up to 8 keywords; at least 5 are mandatory.
\end{abstract}
