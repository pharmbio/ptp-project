...
steps easier to implement and faster to process, given the relative large
size of the uncompressed ExcapeDB data file (18 GB).

From this dataset, all rows for which there were rows with a conflicting
value for the activity flag, were completely removed.
...
i) ``KCNE1'' was replaced with ``MINK1'' which is present in ExcapeDB.

ii) CHRNA1 (coding for the $\alpha1$ subunit of the Acetylcholine receptor) is
skipped, as it was not found in the dataset.

iii) CHRNA4 (coding for the $\alpha4$ subunit of the Acetylcholine receptor) is
included though.

iv) Both MINK1 and CHRNA4 were removed in the filtering step mentioned above,
since the dataset did not contain more than 100 active and 100 non-active
compounds for MINK1 nor CHRNA4.
