...
%
For full information about the pre-processing done by the ExCAPE-DB authors,
see~\cite{Sun2017}. As a help to the reader we note that the activity flag is
-- in the ExCAPE-DB dataset -- set to active (or ``A'') if the
dose-response value in the binding assays was lower than 10 ${\mu}M$ and
non-active (or ``N'') otherwise.
...
dataset contained less than 10\,000 non-active compounds, which makes them
stand out from the other datasets, and where some of them contain a
problematically low amount of non-actives. These 21 targets are referred to
...
active for the target, thus being ``assumed non-active''. These target datasets
are marked with a \checkmark\ in the ``Assumed non-actives added'' column of
...
same number of compounds as the actives. Instead we use an active:non-active
ratio of 1:2 between the classes. The justification for this is that the assumed non-actives likely
have chemistry coming from a larger chemical space compared to the known compounds, 
thus by adding more of the assumed non-actives we can hopefully increase the number 
of examples in the regions of chemical space that are of interest for separating the two classes.
...
\begin{center}
\includegraphics[width=0.8\textwidth]{figures/fig1_datasets.pdf}
    \caption{Graphical overview over how the raw datasets used in this study
    were created. The blue funnel symbol and text represent filtering steps,
    while the barrel and document symbols represent datasets. The criteria
    for the filtering steps are shown with blue text. ``A'' represents
    ``Actives'', and ``N'' represents ``Non-actives''.}
    \label{fig:datasets}
\end{center}
\end{figure}
...
estimates. In CP a prediction region contains the true
...
relates to the \emph{strangeness} of the test example, and is an alternative to the
concept of a model's \emph{applicability domain}~\cite{Norinder:2014fe}. For the classification
...
Conformal Prediction as originally invented, was described for the online transductive setting, meaning that the
underlying learning model had to be retrained for every new test object. Later it was adapted for
the off-line inductive setting too, where the underlying model is trained only once for a batch of training examples.
The Inductive Conformal Predictor (ICP), which is used in this study, require far less computational resources,
but has the disadvantage that a part of the training set must be set aside as a \textit{calibration set}.
The remaining data, called \textit{proper training set}, is used to train the learning model.
As the partitioning of data into a calibration set and proper training set can have a large influence on the performance
of the predictor, it is common to redo this split multiple times and train an ICP
for each such split. This results in a so called Aggregated Conformal Predictor (ACP) that aggregates the predictions for
each individual ICP.
...
generated with height 1-3, which means that molecular sub-graphs including all atoms
of distance 1, 2 or 3 from initial atoms, are generated. Support vector
...
documentation~\cite{CPSignDocs}. By using this parameter the additional
compounds are only added to the proper training set, thus being used for training
the underlying SVM model, but not for the calibration of the predictions. This
ensures that potentially non-typical chemistry in the additional assumed non-active compounds
does not affect the calibration of the predictions in a negative way.
...
\emph{class-averaged observed fuzziness} ($CAOF$) as:
\begin{equation}
CAOF = \frac
...
%\vspace{2\baselineskip}
\begin{center}
\includegraphics[width=0.63\textwidth]{figures/fig2_schematic_workflow.pdf}
    ...
    detailed workflow plots, see Supplement 1, figures S4 and S5.}
...
available in Supplement 1, figures S4 and S5).
...
together with audit log files produced by SciPipe, containing execution
traces of the workflow (all the shell commands and parameters) used to
produce them, are available for download at~\cite{ModelsZenodo}. The models
can be run if obtaining a copy of the CPSign software and a license, from
Genetta Soft AB.
...
found in the Supplement 1 (figure~S1).
...
\begin{figure}[h!]
    \begin{minipage}[c]{0.3\textwidth}
        ...
        \includegraphics[width=\textwidth]{figures/fig3a_pde3a_calib.pdf}
        \label{fig:calplots:a}
        \subcaption{}
    \end{minipage}
    \begin{minipage}[c]{0.3\textwidth}
    \includegraphics[width=\textwidth]{figures/fig3b_slc6a2_calib.pdf}
        \label{fig:calplots:b}
        \subcaption{}
    \end{minipage}
    \begin{minipage}[c]{0.3\textwidth}
        \includegraphics[width=\textwidth]{figures/fig3c_htr2a_calib.pdf}
...
\begin{figure}[h!]
    \begin{minipage}[t]{0.62\textwidth}
        \setcounter{subfigure}{0} % Ensures that subfigures are labeled from a, b, c...
        \includegraphics[width=\textwidth]{figures/fig4a_21small_orig.pdf}
    ...
    \end{minipage} \\
    \begin{minipage}[t]{0.62\textwidth}
        \includegraphics[width=\textwidth]{figures/fig4b_21small_fill.pdf}
    ...
    \begin{minipage}[t]{0.38\textwidth}
        \includegraphics[width=\textwidth]{figures/fig4c_10large.pdf}
...
each target individually, see Supplement 1, figure S2 and S3.
Because of the fact that CP produces sets of predicted labels, including \emph{non},
and \emph{both} in this case, the common sensitivity and specificity measures
do not have clear definitions in this context. Because of this, we have not
included calculated values for them but have instead included compound counts
for the predicted label sets in \ref{fig:valplots} summarized for all
targets, and as CSV files in Supplement 2 (for 0.8 confidence) and 3 (for 0.9
confidence), for each target specifically.

\begin{figure}[h!]
\begin{minipage}[c]{0.38\textwidth}
    \setcounter{subfigure}{0} % Ensures that subfigures are labeled from a, b, c...
    \subcaption[Predicted versus observed labels, for all targets at confidence level 0.8]{
    \includegraphics[width=1\textwidth]{figures/fig5a_alltargets_0p8_valplot.pdf}
    }\label{fig:valplots:a}
\end{minipage}
\hspace{0.10\textwidth}
\begin{minipage}[c]{0.38\textwidth}
    \subcaption[Predicted versus observed labels, for all targets at confidence level 0.9]{
        \includegraphics[width=1\textwidth]{figures/fig5b_alltargets_0p9_valplot.pdf}
...
\end{minipage}
    ...
    labels for each target individually, see Supplement 1,
    figure S2 and S3.}
    \label{fig:valplots}
\end{figure}
...
\begin{figure}
    \begin{minipage}{0.70\textwidth}
        \setcounter{subfigure}{0} % Ensures that subfigures are labeled from a, b, c...
        \includegraphics[height=0.37\textheight]{figures/fig6a_terbutaline.png}
    ...
    \begin{minipage}{0.25\textwidth}
        \fbox{\includegraphics[height=0.23\textheight]{figures/fig6b_terbutaline2.png}}
        ...
        molecular fragments (of height 1-3) that contributed most to the larger
...
lower p-value of the two~\cite{Saunders1999}.  This means that even if a prediction has
...
four highest active predictions in the profile.

\begin{figure}
\hfill
\begin{minipage}[t]{0.3\textwidth}
    \setcounter{subfigure}{0} % Ensures that subfigures are labeled from a, b, c...
    \includegraphics[width=\linewidth]{figures/fig7a_chembl95.png}
...
\hfill
\begin{minipage}[t]{0.3\textwidth}
    \includegraphics[width=\linewidth]{figures/fig7b_chembl550.png}
    \subcaption{The profile for Pilocarpine, a muscarinic acetylcholine
    receptor M$_1$ agonist, with only two moderately higher peaks for active
    prediction, CHRM1 and LCK.}
    \label{fig:threeprofiles:b}
\end{minipage}
\hfill
\begin{minipage}[t]{0.3\textwidth}
    \includegraphics[width=\linewidth]{figures/fig7c_chembl531.png}
    \subcaption{The profile for Pergolide, a DRD1, DRD2, HTR1A, and HTR2A
    agonist, which is reflected by the four highest p-values for an active
    prediction.}
...
valid, with the calibration plots in figure~\ref{fig:calplots}.
...
plots in figure~\ref{fig:valplots}, we see that there is a somewhat higher
fraction of observed non-actives (``N'') correctly predicted as non-actives,
