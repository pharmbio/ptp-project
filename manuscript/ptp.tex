...
The resulting set (named Dataset1. See table~\ref{tbl:datasets} for
an overview of all datasets) consisted of 31 targets (marked as included in
...
These 21 targets were called Dataset2.
%
For these 21 targets making up Dataset2, we extended their respective datasets with randomly
...
A web page federating all the models was also created. The OpenAPI
...
active or non-active for each of the targets, at the chosen confidence. On
theright hand is a graphical profile in the form of a bar plot where confidence of
...
\begin{figure}
    \begin{minipage}{0.70\textwidth}
        ...
        molecule, selects a confidence, whereafter the profile updates
        underneath.}
...
\textit{confidence} of a prediction is defined as 1-`lowest p-value' \cite{Vovk2005}
- meaning that even if a prediction has one high p-value, its confidence and hence 
usefulness in a decision setting might be low. Figure~\ref{fig:threeprofiles:c} 
shows the target profile for Pergolide,
