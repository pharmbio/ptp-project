...
interactions with models trained on data from 31 targets in the ExCAPE-DB
...
chemogenomics~\cite{Bredel2004}, which has emerged and grown over the last decade or two. It is
...
& Dataset4 & The external test set created by extracting rows from ExCAPE-DB for a selected set of \\
...
We based this study upon data from the ExCAPE-DB dataset~\cite{Sun2017}
(EDB). EDB combines data about ligand-target binding from ChEMBL, with
similar data from PubChem. Importantly, PubChem contains ... \todo{Continue...}
...
(gene symbol) and SMILES string, were completely removed. % (See
...
note that the activity flag is set to active (or \texttt{A}) if the
...
The resulting set (named Dataset1) consists of 31 targets (marked as included in
...
These 21 targets are referred to as Dataset2, and their respective target datasets were expanded with randomly
selected examples from the ExCAPE-DB dataset which were not reported to be active for
